...
EyeScanLink::EyeScanLink(std::string linkName, boost::program_options::parsed_options PO) {
  ...
  for(int i = 0; i < numOptions; i++) {
	...
	this->outfile = PO.options[i].value[0].c_str();
      ...
      {
	// found max prescale 
	this->maxPrescale = PO.options[i].value[0];
      }
  }
}
...
void EyeScanLink::printInfo() {
  ...
  syslog(LOG_INFO, "   phase: %s\n"         , (this->phase).c_str());
  syslog(LOG_INFO, "   voltage: %s\n"       , (this->voltage).c_str());
  syslog(LOG_INFO, "   data file name: %s\n", (this->outfile).c_str());
  syslog(LOG_INFO, "   max prescale: %s\n"  , (this->maxPrescale).c_str());
  syslog(LOG_INFO, "\n\n");
}
...
void EyeScanLink::scan() {
  // empty the vector
  (this->esCoords).clear();
  // perform scan  
  this->esCoords = SM->EyeScan(this->name, this->phase, this->voltage, this->mexPrescale);
